...
//  Env: SUPABASE_SERVICE_ROLE (di Render.com)
// ══════════════════════════════════════════════════════════════
...
// Semua aksi berikut butuh login
if ($action !== '' && $action !== 'login' && !isLoggedIn()) {
...
if ($action === 'update_profile' && isLoggedIn()) {
    ...
    // Cek apakah ada file avatar PNG upload
    if (!empty($_FILES['avatar_file']['tmp_name'])) {
        ...
        if (in_array($mime, $allowed, true)) {
            ...
            if ($url) {
                // Simpan sebagai URL reference bukan base64 — tapi tetap support base64 juga
                $data['avatar_data'] = $url;
                $data['avatar_type'] = $ext;
            } else {
                $message = 'Gagal upload avatar ke storage.';
                $msgType = 'error';
            }
        }
    } elseif (!empty($_POST['avatar_base64'])) {
        // Paste base64 langsung
    ...
    if ($res['code'] >= 200 && $res['code'] < 300) {
        ...
        $message = 'Gagal memperbarui profil: ' . json_encode($res['data']);
        $msgType = 'error';
    }
}

// --- ADD LINK CARD ---
if ($action === 'add_link' && isLoggedIn()) {
    checkCsrf();
    ...
    if ($res['code'] >= 200 && $res['code'] < 300) {
        $message = 'Link card berhasil ditambahkan.';
    } else {
        $message = 'Gagal menambah link card.';
        $msgType = 'error';
    }
}

// --- UPDATE LINK CARD ---
if ($action === 'update_link' && isLoggedIn()) {
    checkCsrf();
    $id = (int)($_POST['id'] ?? 0);
    ...
    if ($res['code'] >= 200 && $res['code'] < 300) {
        $message = 'Link card berhasil diperbarui.';
    } else {
        $message = 'Gagal memperbarui link card.';
        $msgType = 'error';
    }
}

// --- DELETE LINK CARD ---
if ($action === 'delete_link' && isLoggedIn()) {
    checkCsrf();
    $id  = (int)($_POST['id'] ?? 0);
    $res = sb('/link_cards?id=eq.' . $id, 'DELETE', null, true);
    if ($res['code'] >= 200 && $res['code'] < 300) {
        $message = 'Link card berhasil dihapus.';
    } else {
        $message = 'Gagal menghapus link card.';
        $msgType = 'error';
    }
}

// --- REORDER ---
if ($action === 'reorder' && isLoggedIn()) {
    checkCsrf();
...
.field input:focus,.field textarea:focus,.field select:focus{
  border-color:rgba(79,128,255,.45);
  box-shadow:0 0 0 3px rgba(79,128,255,.08);
}
...
.toast{
  ...
  animation:slideIn .35s var(--ease-expo) both;
...
.nav-item{
  ...
  padding:10px 20px;font-size:.78rem;
  color:var(--mist);cursor:pointer;
  transition:color .2s,background .2s;
  border-left:2px solid transparent;
}
.nav-item:hover,.nav-item.active{
  color:var(--ice);background:rgba(79,128,255,.05);
...
.panel-card::before{
  content:'';position:absolute;top:0;left:0;right:0;height:1px;
  background:linear-gradient(90deg,transparent,rgba(79,128,255,.4),transparent);
  opacity:.6;
}
.card-title{
  font-family:var(--ff-head);font-size:.85rem;font-weight:600;
...
.link-item{
  ...
  transition:border-color .25s;cursor:grab;
}
...
.badge{
  ...
  font-size:.6rem;font-family:var(--ff-mono);letter-spacing:.1em;text-transform:uppercase;
}
...
/* ── INPUT FILE STYLED ─────────────────────── */
.file-drop{
  border:1px dashed var(--ghost);border-radius:4px;
  padding:20px;text-align:center;cursor:pointer;
  transition:border-color .25s,background .25s;font-size:.78rem;color:var(--mist);
}
...
.modal-overlay{
  ...
  background:rgba(3,4,10,.92);backdrop-filter:blur(20px);
...
.modal-box{
  ...
  background:rgba(10,15,28,.98);border:1px solid var(--edge);
  border-radius:6px;padding:36px;
  transform:translateY(24px);transition:transform .35s var(--ease-expo);
  position:relative;
}
...
.icon-type-btn{
  ...
  border:1px solid var(--edge);color:var(--mist);background:transparent;
  transition:all .2s;
}
...
<div class="login-wrap">
  <div class="login-card">
    ...
    <?php if ($message): ?>
    <div class="toast error" style="position:relative;top:auto;right:auto;margin-bottom:20px;animation:none">
...
<div class="admin-wrap">
  ...
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav">
      <div class="nav-item active" onclick="showSection('profile',this)">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/>
        </svg>
        Profil
      </div>
      <div class="nav-item" onclick="showSection('links',this)">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="panel-card">
          ...
          <div class="grid2">
            ...
            <div class="field">
              <label>Handle</label>
              <input type="text" name="handle" value="<?= htmlspecialchars($profile['handle'] ?? '@robertusdanan') ?>" required
          ...
          <div class="field">
            ...
            <div class="hint">Ditampilkan di bawah nama — contoh: Creator · Developer · Dreamer</div>
          </div>
        </div>
        ...
        <div class="panel-card">
          ...
          <?php
            ...
            $avSrc  = '';
            if (!empty($avData)) {
              if (str_starts_with($avData, 'http')) {
                $avSrc = htmlspecialchars($avData);
              } else {
                $avSrc = 'data:image/' . $avType . ';base64,' . $avData;
              }
            }
          ?>
          ...
          <div id="avatarTabUpload" class="tab-panel active">
            <div class="file-drop" onclick="document.getElementById('avatarFileInput').click()">
              <input type="file" name="avatar_file" id="avatarFileInput" accept="image/png,image/jpeg,image/webp"
                     onchange="previewAvatar(this)"/>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-bottom:6px;display:block;margin:0 auto 8px">
          ...
          <div id="avatarTabBase64" class="tab-panel">
            <div class="field">
              ...
              <div class="hint">Bisa paste dengan prefix data:image/... atau string base64 murni</div>
            </div>
            <div class="field">
        ...
        <div id="customIconArea">
          ...
          <div id="iconTabUpload" class="tab-panel active">
            <div class="file-drop" onclick="document.getElementById('iconFileInput').click()">
              <input type="file" name="icon_file" id="iconFileInput" accept="image/png,image/jpeg,image/webp,image/svg+xml"
...
function switchAvatarTab(tab, el){
  ...
  document.getElementById('avatarTab'+tab.charAt(0).toUpperCase()+tab.slice(1)).classList.add('active');
}
...
function setAvatarPreview(src){
  ...
  wrap.innerHTML = '<img src="'+src+'" alt="Avatar" id="avatarImg" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>';
}
...
function switchIconTab(tab, el){
  ...
  el.classList.add('active');
  document.getElementById('iconTab'+tab.charAt(0).toUpperCase()+tab.slice(1)).classList.add('active');
}
...
function openEditModal(link){
  ...
  setIconType(type, matchBtn || document.querySelector('.icon-type-btn'));

  // Preview current icon if custom
...
(function(){
  ...
  list.addEventListener('dragover', e=>{
    ...
    if(isAfter) target.after(dragged); else target.before(dragged);
  });
  function saveOrder(){
